...
- **Bedienelement ist `<app-theme-menu>`** (Popover nach §7.1, `role="menuitemradio"`), in der Kopfzeile neben dem Sprachumschalter, mobil in derselben Disclosure. Bewusst kein durchklickender Icon-Button: dessen nächster Zustand ist nicht ansagbar.
- **Referenz:** `web/src/app/core/theme/theme.service.ts` (+ `theme.service.spec.ts`), `web/src/app/shared/ui/theme-menu.ts`, `web/public/theme-init.js`, `web/src/index.html`, Flow-Test `web/e2e/theme.spec.ts`.
...
- **Auf einem groben Zeiger ist derselbe Dialog ein Bottom-Sheet — dieselbe Instanz, zweite Erscheinung, kein zweiter Overlay-Stack.** Eine `@media (pointer: coarse)`-Regel auf genau der Pane-Klasse ändert Andockung (`align-items: flex-end`), Breite (voll statt `28rem`) und Höhe (`85dvh` statt `calc(100dvh - 2rem)`); `openAppDialog()` und `dialog-shell.ts` bleiben unverändert, `PointerModeService.isCoarse` entscheidet live statt einmalig beim Öffnen. `DialogShell` rendert dafür einen sticky Ziehgriff mit dem Attribut `data-sheet-handle` — die `SheetDrag`-Direktive sucht exakt danach, um eine Ziehgeste zuzulassen; ändert sich der Name auf einer Seite ohne die andere, bricht Drag-to-Dismiss lautlos (abgedeckt durch `web/e2e/touch-mobile.e2e.spec.ts`). Die Pane bleibt auch hier der Scroll-Container, siehe die CDK-Fallen unten.
- **Drag-to-dismiss** (`shared/ui/sheet-drag.ts` + `sheet-drag-policy.ts`) schließt über `DialogRef.close()`, ist also kein vierter Ausgang neben Backdrop-Tap und Escape. Die Geste beginnt nur am Griff oder wenn die Pane oben angeschlagen ist — sonst bedeutet ein Zug nach unten „Inhalt hochscrollen". Die Schwelle hat **drei** Zahlen, nicht zwei: `distancePx ≥ 96` **oder** `velocityPxPerMs ≥ 0,5`, aber erst ab einer Mindeststrecke von `24 px` — ohne die Untergrenze schlösse ein Vier-Pixel-Zucken beim Tippen das Sheet, weil eine kurze Bewegung per Definition schnell ist.
...
- **Das 44-px-Komfortziel ist inzwischen kein Popover-Sonderfall mehr, sondern an der Button-Größe `lg` verankert** (`shared/ui/button.ts`, `SIZE_CLASSES.lg = 'min-h-11 …'`): jeder flow-tragende Button — Dialog-Bestätigungen, der Mass-Delete-Auslöser, der Schließen-Weg des Sheets — trägt die 44-px-Höhe damit automatisch, ohne dass eine Aufrufstelle sie noch einzeln nachbauen muss. `md` bleibt unverändert für dichte Toolbars, wo eine Maus der realistische Zeiger ist.
...
- **z-Leiter (verbindlich):** Zeilen-Action-Container/Stretched-Link `z-10` (§2.3) < Sticky-Leisten `z-20` < Shell-/Landing-Header und `.app-dock` `z-30` (die Mobile-Disclosure des Headers liegt als `z-20` **im** Header-Kontext und damit über allem). Dropdowns, die aus einer Sticky-Leiste heraus öffnen (z. B. das Zeitraum-Menü der Usage-Stats, `shared/ui/popover.ts` via `shared/datetime/date-range-menu.ts`), erben deren `z-20`-Kontext und liegen damit über dem Content; Dropdowns im Content (Datetime-Picker im Create-Formular, `z-30` im `z-10`-Zeilen-Kontext) bleiben unter den Leisten — sie öffnen nach unten, weg davon.
