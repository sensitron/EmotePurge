...
**What that check can and cannot see.** It reads `/api/channels/{name}/emotes`, which serves **our** database, not 7TV live. It closes the dialog-snapshot gap; it does not close the gap between our mirror and 7TV itself, which is only as fresh as the last resync. A failed fetch is reported as unverified rather than as nothing-to-skip, following the rule already stated next to the same trade-off in `import-target-loader.ts`: a failed check must read as "not verified", never as a false all-clear. That matters more here than usual, because the duplicate it fails to catch cannot be repaired afterwards.
