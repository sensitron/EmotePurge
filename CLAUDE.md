...
| **E** | Launch-Vorbereitung: Ressourcenlimits, Container-Healthchecks (S3-35), Log-Aggregation/Alerting (S3-36), Rechtstexte | 🟡 laufend — **Monitoring selbst ist gebaut** (Admin-Bereich + `GET /api/admin/health` + `WorkerHealthPublisher`), seit der VPS-Härtung 2026-08-04 existieren externer Uptime-Check (Uptime Kuma + Telegram), Backup-Kette VPS→NAS→OneDrive und Docker-Log-Rotation, seit 2026-08-05 Ressourcenlimits (S2-21), Migration-Fail-Fast (S3-34) und getrennte Dev-Compose-Namen (S3-38); offen bleiben Container-Healthchecks + öffentlicher Health-Endpoint, Dead-Man's-Switch/Log-Aggregation, Rechtstexte |
