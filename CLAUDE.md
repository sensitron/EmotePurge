...
| Modul | Inhalt | Stand |
|---|---|---|
| **1 / A** | Chat-Analytics & Live-Synchronisation: Channel-Join/Leave über Redis, anonymes Twitch-IRC, 7TV-Vollsync im Minutentakt, Chat-Matching + 30-Sekunden-Batch-Flush, Worker-Health | ✅ vollständig |
...
- **`EmotePurge.Worker`** — .NET Worker Service mit fünf Hosted Services: `Worker` (Boot-Recovery + Redis-Kommandos), `UsageFlushWorker`, `SevenTvPeriodicResyncWorker`, `TwitchConnectionWatchdog`, `WorkerHealthPublisher`. `TwitchChatManager` kapselt genau **einen** langlebigen `TwitchLib.Client` für die gesamte Worker-Lebensdauer — **nie** pro Channel/Join neu instanziieren; die Reconnect-Entscheidung liegt TwitchLib-frei in `ReconnectPolicy`.
...
11. **Neue Services/Logik in `EmotePurge.Infrastructure` bekommen einen Test** in `tests/EmotePurge.Infrastructure.Tests` (Fixture pro externer Abhängigkeit unter `Fixtures/`, `Integration/` oder `Unit/` je nachdem, ob echte Infrastruktur gebraucht wird).
