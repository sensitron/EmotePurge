...
- **`EventSource` is never constructed directly — always go through the `EVENT_SOURCE_FACTORY` InjectionToken** (`core/live/event-source.factory.ts`). SSE streams bypass `HttpClient` entirely, so neither the `apiAuthInterceptor` (401 handling) nor `HttpTestingController` apply to them; the factory token is what makes `LiveUpdateService` stubbable in Vitest, and Playwright specs stub `window.EventSource` via `installLiveStub` in `e2e/support/mocks.ts`. Live events are thin notifications (`{type, channel, sessionId?}`) — react by re-fetching through the existing REST services, never by trusting event payloads as data. The release call on the way out goes through its own `LIVE_CONNECTION_RELEASE_FACTORY` InjectionToken (`core/live/live-connection-release.factory.ts`), same reason: it fires a `fetch(..., { method: 'DELETE', keepalive: true })` that must bypass `HttpClient`/`apiAuthInterceptor` too (see DECISIONS 2026-09-11, #128).
- **Tests: behaviour yes, template no.** Services, guards and pure utilities under `core/` and `shared/` get a co-located `*.spec.ts` (Vitest; `HttpTestingController` for anything with `HttpClient`, `TestBed.runInInjectionContext` for guards). A component gets one too **when it carries non-trivial decision logic of its own** — a purely presentational one does not. Assert state transitions and `computed()` results, outputs and dialog return values, blocking decisions together with the reason given for them, accessibility semantics (role, accessible name, `aria-*` wiring), and the order of rows or notices **where that order is documented as a contract elsewhere** (`docs/UI-Designsprache.md` §7.2). Do not assert CSS classes, Tailwind chains, non-semantic tags or nesting, snapshots, or a translation's exact wording for its own sake — wording may identify which notice appeared, it is not the subject under test. Rule of thumb: if a redesign may change it without a user experiencing anything different, it does not belong in a spec. Set signal inputs with `fixture.componentRef.setInput()` and never read a required input in a constructor (NG0950). Flows spanning several pages get a Playwright E2E spec with mocked `/api/**` responses on top, not instead. This is root `CLAUDE.md` rule 12.
