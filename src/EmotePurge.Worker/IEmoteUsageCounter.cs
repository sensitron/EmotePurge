...
{
    void Increment(string emoteId, UsageCategory category);
    ...
    // and by the shared-chat split: an emote seen only from bots, or only from foreign rooms,
    // still counts as one buffered emote.
