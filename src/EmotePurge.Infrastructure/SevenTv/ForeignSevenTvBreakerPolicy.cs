...
public readonly record struct ForeignSevenTvBreakerDecision(bool Allowed, bool OpenedByRateLimit, TimeSpan RemainingOpenTime);
...
/// <c>Retry-After</c> header, when present) overrides <see cref="DefaultOpenDuration"/> — honoring
/// what the provider actually asked for beats guessing.
/// </para>
/// <para>
...
public sealed class ForeignSevenTvBreakerPolicy(TimeProvider? timeProvider = null)
{
    ...
    /// to call <see cref="RecordSuccess"/> or <see cref="RecordFailure"/> exactly once with the
    /// outcome — otherwise the breaker never releases <see cref="_probeInFlight"/> and stays stuck
    /// open past its own duration.
    /// </summary>
    public ForeignSevenTvBreakerDecision TryAcquire()
    {
        lock (_gate)
        {
            if (!_open)
            {
                return new ForeignSevenTvBreakerDecision(true, false, TimeSpan.Zero);
            }

            var remaining = _openUntil - _timeProvider.GetUtcNow();
            if (remaining > TimeSpan.Zero)
            {
                return new ForeignSevenTvBreakerDecision(false, _openedByRateLimit, remaining);
            }

            if (_probeInFlight)
            {
                return new ForeignSevenTvBreakerDecision(false, _openedByRateLimit, TimeSpan.Zero);
            }
    ...
    /// <summary>A 7TV call succeeded: closes the breaker (if open) and resets the failure streak.</summary>
    public ForeignSevenTvBreakerTransition RecordSuccess()
    {
        lock (_gate)
        {
            var wasOpen = _open;
            ...
            return wasOpen ? ForeignSevenTvBreakerTransition.Closed : ForeignSevenTvBreakerTransition.None;
        }
    }

    /// <summary>
    /// A 7TV call failed. <paramref name="outcome"/> decides which of the two E4 triggers applies;
    /// <paramref name="retryAfter"/> is only read when the breaker actually opens as a result.
    /// </summary>
    public ForeignSevenTvBreakerTransition RecordFailure(ForeignSevenTvBreakerOutcome outcome, TimeSpan? retryAfter)
    {
    ...
    /// 7TV, such as the provider-wide budget (<see cref="ForeignEmoteSetProviderBudget"/>) timing out
    /// first, or an upstream answer that never touched 7TV at all (the Twitch-side failures a foreign
    /// lookup can also end in). Idempotent and safe to call unconditionally: a no-op whenever the
    /// breaker was not actually mid-probe (closed, or a different caller already owns the probe).
    /// </summary>
    public void ReleaseProbeWithoutOutcome()
    {
        lock (_gate)
        {
            _probeInFlight = false;
